#!/usr/bin/env python
"""
Run walk-forward backtest for all hitter stats (K%, BB%, HR/PA, xwOBA).

Usage
-----
    # Quick (fewer draws, faster but noisier)
    python scripts/run_hitter_backtest.py --quick

    # Single stat only
    python scripts/run_hitter_backtest.py --quick --stat k_rate
...
def main() -> None:
    ...
    summary = run_hitter_backtest(stats=stats, folds=FOLDS, **sampling)
    ...
    logger.info("Results saved to outputs/hitter_multi_stat_backtest.csv")
